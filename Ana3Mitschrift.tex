...
\item Das \emph{Tangentialbündel} $TM$ ist die \emph{disjunkte} Vereinigung der Tangentialräume $TM=\bigsqcup\limits_{p\in M}T_p M$ zusammen mit der Fußpunktprojektion $\pi:TM\to M$.
\end{enumerate}
\end{mdframed}
\end{definition}
Die Tangentialräume an verschiedenen Punkten sind also disjunkt. Sie tragen natürliche Strukturen als \emph{lineare Räume}. Weil die Differentiale $\D(\underbrace{\widetilde{x}\circ x^{-1}}_{\mathcal{C}^k})$ der Kartenwechsel lineare Automorphismen sind, existiert angesichts \eqref{unabh-karte} sowie der Kettenregel auf $T_pM$ eine eindeutige lineare Struktur als $m$-dim Vektorraum, sodass die Bijektionen
...
\end{small}
\end{center}
Ist $(\widetilde{U},\widetilde{x})$ eine weitere Karte, so ist der Kartenwechsel
...
\end{array}
\end{equation*}
Bezeichnen wir suggestiver die Koordinaten in $M$ bzgl. der Karten mit $x$ bzw $\widetilde{x}$ und den Kartenwechsel mit $\widetilde{x}(x)$, so nimmt der Kartenwechsel auf $TM$ die Form
$$(x,v)\longmapsto (\widetilde{x}(x),\D\widetilde{x}_x(v))$$
an. Das Differential einer Abbildung $\widetilde{x}(x)$ zwischen offenen Teilmengen euklidischer Räume, ist explizit für $v=\sum\limits_{j=1}^m x_j e_j$ gegeben durch
...
Aus der letzten Formel \eqref{basis-darst-kartenwechsel} lesen wir die Basis in den Tangentialräumen ab. Für $v=e_j$ entspricht  die letzte Formel \eqref{basis-darst-kartenwechsel} der Richtungsableitung $\dfrac{\del}{\del x_j}\bigg|_p$:
$$\frac{\del}{\del x_j} \bigg|_p = \sum_{i=1}^m \frac{\del \widetilde{x}_i
}{\del x_j}(x(p))\frac{\del}{\del \widetilde{x}_i}\bigg|_p$$
bzw.
...
\end{equation}
und vom Tangentialbündel $TW$
\begin{equation}\label{ident-buendel}
TW=\bigsqcup_{p\in W}T_pW \overset{\cong}\longrightarrow W\times \R^m, [c]\longmapsto (c(0,c'(0)).
\end{equation}
Das Standardbasenfeld ist gegeben durch
...
\rule{\textwidth}{0.4pt}\\
Wir approximieren jetzt differenzierbare Abbildungen lokal durch lineare Abbildung der Tangentialräume, indem wir den Begriff des Differentials für Abbildungen von Untermannigfaltigkeiten verallgemeinern.
...
also hängt $[F\circ c]$ nur von $[c]$ ab.\vspace{0.4pc}\\
Das Differential bildet Tangentialräume auf Tangentialräume ab:
\begin{center}\vspace{0.2pc}
...
\end{tikzcd}
\end{center}
Insbesondere ist $dF_p=dF|_{T_pM}$ als Komposition linearer Abbildung wieder linear. Im Fall von Abbildungen offener Teilmengen euklidischer Räume ist das Differential konsistent mit dem früher eingeführten, denn letztes bildet 
...
\rule{\textwidth}{0.4pt}
Wie \eqref{karte-von-tanb} beschreibt, induziert eine Karte $(U,x)$ eine Bündelkarte $(TU,Tx)$.
\begin{center}
\begin{tikzcd}
...
\end{tikzcd}
\end{center}
Die induzierten Bündelkarten $T x$ sind Differentiale der Karten $\D x$ auf Mannigfaltigkeiten und die Bündelkartenwechsel $T\widetilde{x}\circ (Tx)^{-1}$ stimmen bis auf derselben Identifikationen überein mit den Differentialen der Kartenwechsel $\widetilde{x}\circ x^{-1}$ auf $M$.\\\\
\emph{In lokalen Koordinaten:} Das Differential $dF_p$ ist relativ zu den Koordinatenbasen $\dfrac{\del}{\del x_j}$ bzw. $\dfrac{\del}{\del y_i}$ ist gegeben durch die Jacobische durch die Jacobische der lokalen Koordinatendarstellung:
...
Weiter liefert \eqref{basis-darst-kartenwechsel}: $F$ von der Klasse $\mathcal{C}^k$ $\implies$ $\D F$ von der Klasse $\mathcal{C}^{k-1}$.

\subsection{Totales Differential, 1-Formen und Kotangentialbündel}
Sei $M$ eine $\mathcal{C}^k$-Mannigfaltigkeit, $k\geq 1$. Ist $f: M\to \R$ differenzierbar, so heißt die Abbildung $$\D f:TM\to T\R$$ das \emph{totale Differential} von $f$. Das totale Differential ist also die Familie der linearen Abbildungen $$\D f_p:T_pM\to T_{f(p)}\R\cong \R.$$
Sie sind Linearformen auf Tangentialräumen, d.h. $\D f_p \in (T_p M)^*$.
\begin{definition}[\textbf{Kotangentialraum, Kotangentialbündel}]
\begin{mdframed}\
\begin{enumerate}
...
%\appendix
%
%\section{Multilineare Algebra und Tensorprodukt}
%\paragraph{Multilineare Abbildungen.} Seien $V_1,...,V_k,W$ $K$-Vektoräume. Eine Abbildung
%$$\mu: V_1 \times ... \times V_k \longrightarrow W$$
%heißt $K$-\emph{multilinear}, falls sie linear in jeder Komponente ist. Der Raum
%$$\operatorname{Multi}(V_1,...,V_k;W)$$
%aller multilinearen Abbildungen von $V_1\times ... \times V_k$ nach $W$ ist wieder ein $K$-Vektorraum. Eine multilinare Abbildung ist bereits festgelegt, wenn die Bilder der Basisvektoren schon festgelegt sind.
%\paragraph{Multilinearform.} Eine Multilinearform ist eine multilineare Abbildung in den zugrundeliegenden Körper $K$. Die Determinantenabbildung ist eine normierte alternierende Multilinearform. }
