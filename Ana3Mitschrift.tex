...
\subsection{Messbare nummerische Funktionen}
...
\begin{example}
Sei $X$ ein topologischer Raum versehen mit der Topologie $\TT$. Dann induziert $\TT$ die Borelsche $\sigma$-Algebra $\BB(X)=\sigma(\TT)$ und $f:X \longrightarrow \ol\R$ ist genau dann messbar, wenn $f$ als eine Abbildung Borel-messbar ist. Insbesondere sind stetige nummerische Funktionen messbar.
\end{example}
...
\subsubsection{Integral nichtnegativer Funktionen}
...
\end{split}
\end{equation*}
Die \emph{Monotonie} des Integrals folgt ebenfalls direkt aus der Definition, also aus der Monotonie von Maßen hier für $\mu\otimes \beta^1$:
\begin{equation*}
\begin{rcases}
f,g \in \MM_{\geq 0}\\
f\leq g
...
\end{equation*}
Auch die grundlegende \emph{Konvergenzeigenschaften} des Integrals auf nichtnegativen messbaren Funktionen lassen sich unmittelbar aus Eigenschaften des Produktmaßes ableiten, nämlich aus seinen Stetigkeitseigenschaften als Maß. \\
...
\begin{satz}[\textbf{Satz von der monotonen Konvergenz} (B. Levi)]
\begin{mdframed}
Für eine schwach monoton wachsende Folge $(f_n)_{n \in \N}$ von Funktionen $f_n \in \MM_{\geq 0}(X,\AA)$ gilt
$$
...
$$\lim_{n\to \infty }\int_X \left(\inf_{k\geq n} f_k\right) = \int_X (\liminf_{n\to\infty} f_n) \D \mu \leq \liminf_{n\to \infty}\int_X f_n \D \mu.$$
\end{proof}
\end{satz}
...
\subsubsection{Integral allgemeiner nummerischer Funktionen}
...
$$
Insbesondere auf $\R^d$: Stetige Funktionen $f: \R^d \longrightarrow \R$ mit \emph{kompaktem Träger} $\ol{\{f\neq 0\}}$ sind integrierbar. Denn die Kompaktheit des Trägers impliziert die Beschränktheit von $f$, also $\lambda^d(\ol{\{ f\neq 0\}})< \infty$ und somit die Integrierbarkeit.
Dies gilt allgemeiner für Borel-Maße $\mu$ (d.h. $\forall x \in X \  \exists$ offene Umgebung $U$ mit $\mu(U)<\infty$) auf topologischen Räumen $X$.
\end{example}
\marginpar{\tiny{9.12.2019}}
