...
Sind $\FF_i \subset \mathcal{P}(X_i), i=1,...,n$ Familien von Teilmengen, so entstehen das Produkt von ``Quadern''
\begin{equation*}
\begin{split}
...
\begin{enumerate}[(\roman*),topsep=5pt]
	...
	\item Sind $\RR_i \subset \mathcal{P}(X_i)$ Ringe und $\EE_i \subset \RR_i$ Erzeugendensysteme für $i=1,...,n$, so erzeugt die Familie von Zylindermengen $\mathcal{Z}(\EE_1,...,\EE_n)$ den Produktring.
...
\end{equation*}
Aus der \hyperref[propC]{letzten Proposition} folgt, dass $\QQ^d$ ein Halbring auf $\RR^d$ ist. Es folgt ebenfalls, dass
\begin{equation*}
...
\begin{equation*}\begin{split}
	A_0 & := \emptyset	\\
	\widetilde{A}_{n} &:= \widetilde{A}_{n} \setminus (A_0\cup ... \cup A_{n-1})	 \end{split}
\end{equation*}
...
\begin{enumerate}[(\roman*),topsep=5pt, itemsep = 0 pt]
	\item \emph{$\sigma$-Additivität:} Ist $(A_n)_{n \in \N}$ eine Folge paarweise disjunkter Teilmengen, $A_n \in \RR$ mit $\bigsqcup_nA_n \in \RR$, so gilt
	$$
	\mu \bigg(\bigsqcup_{n=1}^\infty A_n \bigg) = \sum_{n=1}^\infty \mu (A_n)	
	$$
	\item \textit{Stetigkeit von unten:} Ist $(B_n)_{n\in\N}$ aufsteigend, $B_n \in \RR$ mit $B_n \nearrow B \in \RR$, so gilt
	$$
	\mu (B_n) \nearrow \mu (B)	
	$$
\end{enumerate}
	...
	\begin{enumerate}[(\roman*),topsep=5pt, itemsep = 0 pt]
	\item[(iii)] \emph{Stetigkeit von oben:} Ist $(C_n)_{n \in \N}$ absteigend, $C_n \in \RR$ \textbf{mit} $\mu(C_n) < \infty$ \textbf{und} $C_n \searrow C \in \RR$, so gilt
	$$
	\mu(C_n) \searrow \mu (C)	
	$$
	\item[(iv)] \emph{Stetigkeit von $\emptyset$:} Ist $(D_n)_{n \in \N}$ absteigend, $D_n \in \RR$ mit $\mu(D_n) < \infty$ und $D_n \searrow \emptyset$, so gilt
	$$ \mu (D_n) \searrow 0$$
	\end{enumerate}
...
\begin{itemize}
	...
	\item die Familie der offenen Bälle, bzw. offene Bälle mit Radien $\in \QQ^+$ und Zentren $\in \QQ^d$, denn jede offene Teilmenge ist eine abzählbare Vereinigung von offenen Bällen,
...
\end{mdframed}
\begin{proof}
Einerseits: Jede offene Teilmenge von $\R^d$ ist (notwendigerweise) wegen der Dichtheit von $\QQ$ in $\R$ eine abzählbare Vereinigung von Quandern $[a,b)$ aus $\QQ^d$ mit rationalen Ecken, d.h. $a_i<b_i \ \forall i, a,b\in \QQ^d$. Also $\BB^d \subset \sigma (\QQ^d)$.\\
...
\begin{proposition}
\begin{mdframed}
$\cap$-stabile Dynkin-Systeme sind genau $\sigma$-Algebren.
\end{mdframed}
\begin{proof}
$\sigma$-Algebren sind Dynkin-Systeme und $\cap$-stabil, also $\cap$-stabile Dynkin-Systeme. Ist $\DD$ ein $\cap$-stabiles Dynkin-System, so lassen sich abzählbare Vereinigungen als abzählbare \emph{disjunkte} Vereinigungen ausdrücken:
\begin{equation*}
	\begin{split}
		     \bigcup_{n \in \N}A_n
...
\begin{mdframed}
Eine Abbildung $f:(X,\AA) \longrightarrow (Y,\BB)$ von Messräumen heißt \textbf{messbar}, falls $f^{-1}(B) \in \AA \ \forall B \in \BB$, d.h. Urbilder messbarer Teilmengen soll messbar sein. Man sagt auch $f:X\longrightarrow Y$ ist \textbf{$\AA$-$\BB$-messbar}.
\end{mdframed}
\end{definition}
...
$ eine Algebra, die $\FF$ enthält und in $\sigma(\FF)$ enthalten ist, somit Gleichheit. D.h. $f^*(\sigma(\FF)) \subset \sigma(f^*\FF)$.
\end{proof}
\end{lemma}
