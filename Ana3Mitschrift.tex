...
$$
r_* \lambda^d = s_{d-1} \cdot r^{d-1}\lambda^1|_{[0,\infty)},
$$
...
Seien $(X_i,\AA_i,\mu_i)$ Maßräume und $\mu_i$ $\sigma$-endlich, $i=1,2$. Wir wissen aus den früheren Diskussionen, dass es ein eindeutiges Produktmaß $\mu_1\otimes \mu_2$ auf $\AA_1 \otimes \AA_2$ gibt sodass
$$(\mu_1\otimes\mu_2)(A_1\times A_2) =\mu_1(A_1)=\mu_2(A_2), \ \ A_i \in \AA_i.$$
Wir geben jetzt eine Integraldarstellung von $\mu_1\otimes \mu_2$ im Sinne von Cavalieri: Sei $M\subset X_1 \times X_2$. Die Querschnitte $M_{x_1}\subset X_2$ für $x_1\in X_1$ sind definiert durch
$$M\cap (\{x_1\}\times X_2) =: \{x_1\} \times M_{x_1}.$$
%Ist $M\in \AA_1 \otimes \AA_2$, so ist $M_{x_1} \in \AA_2$ für jedes $x_1\in X_1$.

\begin{satz}
\begin{mdframed}
Ist $M\in \AA_1\otimes \AA_2$, so ist $M_{x_1}\in \AA_2$ für jedes $x_1\in X_1$. Weiter ist die Funktion $x_1 \mapsto \mu_2(M_{x_1}) \in [0,\infty]$ messbar, d.h. $\in\MM_{\geq 0}(X_1,\AA_1)$ und es gilt
\begin{equation}
...
Außerdem ist $\FF$ abgeschlossen unter \emph{disjunkten} abzählbaren Vereinigungen. Sind $M_n \in \FF$ für $n\in \N$ paarweise disjunkt, so ist der $X_2$-Querschnitt ihrer Vereinigung für jedes $x_1 \in X_1$ messbar, also 
...
Wir wenden nun frühere Überlegungen zu Dynkin-System an. Zunächst zeigen wir unter der Annahme, dass $\mu(X_i)<\infty$ für $i=1,2$. Dann ist $\FF$ wegen
$$\mu_2(\complement M_{x_1})=\underbrace{\mu_2(X_2)}_{<\infty, \te{ const}}-\mu_2(M_{x_1})$$
...
und somit $M\in\FF$. Somit sind wir fertig mit dem Beweis.
\end{proof}
\end{satz}
...
\begin{example}[\textbf{Kegelvolumen}]
 Sei $K$ ein Kegel mit Grundfläche $B$ und Höhe $h$. Wir zerlegen hier $\R^d = \R^{d-1} \times \R$. Die Querschnitte 
...
\begin{example}[\textbf{Kegelvolumen und $\Gamma$-Funktion}]
Wir wollen das Volumen des $d$-dim euklidischen Einheitsballs 
...
v_d =v_{d-1}\cdot I_d.
\end{equation}
Die Diskussion vom Wallis-Produkt zusammen mit der Substitution $h=\sin u$ liefert
$$I_d = \int_{-\pi/2}^{\pi/2} \cos^d u \D u.$$
...
Durch partielle Integration erhalten wir außerdem die Rekursionsformel
...
\end{equation}
Z.B. gilt $v_4=v_2 \cdot \frac{\pi}{2}=\frac{\pi^2}{2}$. Wir erinnern uns an die Definition der $\Gamma$-Funktion:
...
Seien $(X_i,\AA_i,\mu_i)$ Maßräume, $i=1,2$ und $\mu_i$ $\sigma$-endlich. \vspace{0.5pc}\\Ist $f \in \MM_{\ol\R}(X_1\times X_2, \AA_1 \otimes \AA_2)$, so ist $f(x_1, \cdot) \in \MM_{\ol\R}(X_2,\AA_2)$ für jedes $x_1 \in X_1$.
...
\begin{equation*}
\int_{X_1\times X_2}f \D(\mu_1\otimes\mu_2) = \int_{X_1}\left(\int_{X_2}f(x_1,x_2)\D\mu_2(x_2)\right)\D\mu_1(x_1)
\end{equation*}
\end{mdframed}
\begin{proof}
