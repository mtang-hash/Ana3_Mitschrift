...
$$\frac{d c}{d t}(\tau):= c'(\tau):=[t\mapsto c(t+\tau)]=[c(\cdot +\tau)].$$
\rule{\textwidth}{0.2pt}\\
...
\begin{definition}[\textbf{Differentialform, 1-Form}]
\begin{mdframed}
Eine \emph{Differentialform} vom Grad 1, kurz \emph{1-Form}, auf $M$ ist ein \emph{Schnitt} des Kotangentialbündels $T^*M$, d.h. eine Abbildung $\alpha:M\to T^*M$ mit $\pi_{T^*M}\circ \alpha =\id_M$.
\end{mdframed}
\end{definition}
...
$$df=\sum_{i=1}^{m}\frac{\del f}{\del x_i} dx_i.$$
\rule{\textwidth}{0.2pt}\\
...
\end{equation}
Wir können 1-Formen $\alpha$ auf $N$ \emph{punktweise} zurückziehen zu 1-Formen $F^*\alpha$ auf $M$:
\begin{equation}
...
\subsubsection{Integration von 1-Formen längs Kurven}
...
\begin{mdframed}
Für eine beliebige stetige 1-Form $\alpha$, d.h. die Koeffizientenfunktionen der 1-Form bzgl lokaler Koordinaten sind stetig, definieren wir ihr \emph{Integral längs} einer $\mathcal{C}^1$-Kurve $c:I=[a,b]\to M$ als
