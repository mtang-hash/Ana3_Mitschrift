...
\end{array}
\end{equation*}
wobei $\widehat{x_i}$ bedeutet, dass der Term $x_i$ weglassen wird. Die $\kappa_i$ heißen \emph{affine Koordinaten}, mit der Umkehrabbildung
...
\begin{center}
\includegraphics[scale=0.105]{tanraum.eps}\\
\begin{small}
...
an. Das Differential einer Abbildung $\widetilde{x}(x)$ zwischen offenen Teilmengen euklidischer Räume, ist explizit gegeben durch
$$\D\widetilde{x}_x(v)=\sum_i\sum_j\bigg(\underbrace{\frac{\del\widetilde{x}_i}{\del x_j}}_{\frac{\del(\tilde{x}\circ x^{-1})_i}{\del x_j}}v_i\bigg)e_i.$$
\marginpar{\tiny{16.1.2020}}
\comment{
\newpage
\appendix
