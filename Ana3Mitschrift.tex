...
als den Raum der \textbf{$\boldsymbol\mu$-integrierbaren $\R$-wertigen} Funktionen auf $X$. Dies ist die Vorform der $L^1$.
\vspace{0.5pc}
...
\end{proof}
\end{lemma}
Weil $\mu$-integrierbare Funktionen $\mu$-fast überall endliche Werte annehmen, haben ihre Äquivalenzklassen $\R$-wertige Repräsentanten. Wir gelangen zur natürlichen Identifikation
\begin{equation*}
\begin{split}
L^1(X,\AA,\mu)&:=\LL^1 (X,\AA,\mu)/_{=_\mu}\\&= \{[f]\in\MM_{\ol\R}(X,\AA)/_{=_\mu} \mid \|f\|_1<\infty\} \subset\MM_{\ol\R}(X,\AA)/_{=_\mu}.
\end{split}
\end{equation*}
Da die $\mu$-fast überall verschwindende Funktion, also $[0]$, einen Untervektorraum von $\LL^1$ bilden, ist $L^1$ ein Quotientenraum von $\LL^1$ nach $[0]$, also
$$L^1 \cong \LL^1/{\{f\mid f=_\mu 0\}}.$$
Zusammenfassend:
\begin{satz}\begin{mdframed}
\begin{enumerate}
\item $L^1(X,\AA,\mu)$ ist in natürlicher Weise ein $\R$-Vektorraum.
\item Die Einschränkung von $\|\cdot \|_1$ von $\MM_{\ol\R}/_{=_\mu}$ auf $L^1$ ist eine \emph{Norm}, die sogenannte \emph{1-Norm}.
\item Das $\mu$-Integral steigt ab zu einem wohldefinierten linearen Funktional
\begin{equation*}
\begin{array}{ccc}
L^1(X,\AA,\mu) & \longrightarrow & \R \\
{[}f] & \longmapsto	& \int_X f \D\mu.
\end{array}
\end{equation*}
Es ist 1-Lipschitz bzgl. der 1-Norm $\|\cdot\|$.
\end{enumerate}
\end{mdframed}
\end{satz}
...
\item Die Monotonie des Integrals drückt sich darin aus, dass es auf dem konvexen Kegel der Äquivalenzklassen nichtnegativer Funktionen nichtnegative Werte annimmt.
\end{enumerate}
\end{remark}
...
Sei $V$ ein endlich-dimensionaler $\R$-Vektorraum, $(e_i)$ eine Basis und $(e_i^*)$ die duale Basis von $V^*$. \footnote{Jeder Vektor $v\in V$ hat eine eindeutiger Darstellung als $v= \sum\limits_i \underbrace{e_i^*(v)}_{\in\R}e_i$.} 
Ist $f:X \to V,\lambda\in V^*$ eine Linearform, so setze $\lambda(f):=\lambda \circ f :X \to \R$.  Insbesondere sind $f_i:=e_i^*(f)$ die Komponenten von $f$ bzgl. $(e_i)$, d.h. $f=\sum\limits_i e_i^*(f)e_i$. Für $\lambda=\sum\limits_i \lambda_ie_i^*$ ist $\lambda(f)=\sum\limits_i \lambda_i f_i$.

\begin{definition}
\begin{mdframed}
$f:X\to V$ heißt \emph{messbar}, falls $\lambda(f)$ für alle Linearformen $\lambda\in V^*$ messbar sind. (Dies ist äquivalent dazu, dass alle $f_i$ messbar sind.)
\end{mdframed}
Wir bezeichnen $\MM_V(X,\AA)$ den Raum der messbaren $V$-wertigen Funktionen.\vspace{0.2pc}
\end{definition}
Wir fixieren nun eine Norm $\|\cdot\|_V$ auf $V$. 
\begin{definition}
\begin{mdframed}
$f\in \MM_V$ heißt \emph{$\mu$-integrierbar}, falls\tablefootnote{$\|f\|_1$ hängt von der gewählten Norm $\|\cdot\|_V$ ab. Weil je zwei Normen auf einem endlich-dimensionalen Vektorraum zueinander äquivalent sind, ist unsere Definition wohldefiniert.}
$$\|f\|_1 := \int_X \|f\|_V \D \mu <\infty.$$
\end{mdframed}
Wir bezeichnen $\LL^1_V:=\LL^1(X,\AA,\mu;V)$  den Raum der $\mu$-integrierbaren $V$-wertigen Funktionen.
\end{definition}
...
\end{mdframed}
\end{korollar}
Für $f\in \LL^1_V$ wollen wir jetzt das $\mu$-Integral $\LL^1  \to V, f\mapsto \int_X f \D \mu$ definieren. Weil
\begin{equation*}
\begin{array}{ccc}
V^* & \longrightarrow& \R \\
...
\end{array}
\end{equation*}
ein lineares Funktional auf $V^*$ ist,\footnote{Insbesondere eine Linearform, also $\in  \underset{(\lambda \mapsto \lambda(v)) \mapsfrom v}{(V^*)^* \cong V}$.} existiert ein eindeutiger Vektor $\int_X f \D\mu \in V$ sodass
$$
...
$$
Z.B. für $V=\mathbb{C}$ gilt $$\int_X f\D\mu = \int_X (\Re f)\D\mu +i \int_X (\operatorname{Im} f)\D\mu.$$
\vspace{2pc}
\subsubsection{Der Vergleich mit dem Integral auf Regelfunktionen}
...
\begin{footnotesize}
(Verallgemeinerte Konvergenzsätze für nichtnegative Funktionen, vgl. Monotone Konvergenz und Fatou-Ungleichung.)\vspace{0.5pc}\\
\end{footnotesize}
Wir wollen Situationen definieren, in denen wir die Integration und Limesbildung vertauschen können. Um von punktweiser Konvergenz auf Konvergenz der Integrale zu schließen, müssen wir die Funktionen ``gemeinsam lokalisieren''.
\begin{satz}[\textbf{Satz von der Majorisierten Konvergenz} (\scshape Lebesgue)]
\begin{mdframed}
Sind $f_n,f\in \MM_{\ol\R}$ sodass
$$f_n \longrightarrow f \ \ \mu\te{-fast überall}$$
und existiert $g\in \MM_{[0,\infty]}$ $\mu$-integrierbar mit 
$$|f_n|\leq g \ \ \mu\te{-fast überall},$$
so ist $f$ $\mu$-integrierbar mit
$$\int_X f_n \D\mu \longrightarrow \int_X f \D \mu.$$
\end{mdframed}
\begin{proof}
Durch Abändern von $f_n,f,g$ auf einer $\mu$-Nullmengen dürfen wir ohne Einschränkung annehmen, dass die Bedingungen $f_n \to f$ und $|f_n|\leq g$ überall gelten und die Funktionswerte endlich sind. Also $f_n,f\in \LL^1$, da $|f_n|,|f|\leq g$. Ersetze $f_n$ durch $f_n -f$ mit der gemeinsamen integrierbaren Majorante $|g|+|f|$. O.B.d.A. sei weiter $f=0$, d.h. $f_n \to 0$. Zu zeigen: $\int_X f_n \D\mu\to 0$. Betrachte die Folge $(g-f_n)$. Es ist $0 \leq g-f_n \leq g$ für $n\in \N$ und $g-f_n \to g$. Das Lemma von Fatou liefert
\begin{equation*}
\begin{split}
